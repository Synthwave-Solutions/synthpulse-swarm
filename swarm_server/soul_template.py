...
COMMON_SOUL_TEMPLATE = (
    ...
    "--- TERMINAL WORKING DIRECTORY (read before ANY terminal command) ---\n"
    "CRITICAL: your terminal does NOT start in your workspace, and its working "
    "directory does NOT persist between calls. Each terminal command begins in an "
    "unrelated directory, so a bare 'cd sub' or a relative invocation like "
    "'python -m backend.main' will fail ('No such file or directory', "
    "'ModuleNotFoundError') or silently run in the wrong place. Your file tools "
    "(read_file/write_file) ARE workspace-relative, but the terminal is NOT — never "
    "assume they share a location. For EVERY terminal command, do one of:\n"
    "  - pass the absolute 'workdir' parameter, e.g. workdir={workspace_path}/repo; or\n"
    "  - use absolute paths in the command; or\n"
    "  - chain it all in ONE command: 'cd {workspace_path}/repo && <your command>'.\n"
    "Never rely on a 'cd' from a previous terminal call. For long-lived processes "
    "(dev servers, watchers) pass background=true, then health-check (curl/ps) in a "
    "FOLLOW-UP command rather than backgrounding with '&'.\n\n"
    ...
    "  - Run terminal commands (with an absolute workdir/paths — see TERMINAL "
